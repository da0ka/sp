...
void decode(string& fileName,vs& opt){
	...
	int fileSize, interval;
	fread(&fileSize,sizeof(int),1,infp);
	fread(&interval,sizeof(int),1,infp);
	if(fileName.size()<3 || fileName.substr(fileName.size() - 2,2) != EXT){
		cout << fileName << endl;
	...
	for(vs::iterator i = opt.begin(); i != opt.end(); i++){
		if(*i == "p"){
			...
			if(!tp) throw"tp open error";
			for(int prepare = -1;;){
				int start,last;
				if(!(tp >> start)) break;
				if(prepare >= start) throw"tp start is not good";
				prepare = start;
				tp >> last;
				if(prepare >= last){
					printf("%d %d\n",prepare,last);
	...
	vector<int> outPos, treePos;

	if(!stdoutCheck) printf("%s (original size %d) INTERVAL:%d  read parm...",(fileName.substr(0,fileName.size() - 2)).c_str(),fileSize,interval);
	int rootGsize = ((fileSize + MAXBUF - 1) / MAXBUF);
	{
		...
		for(int i = rootGsize;i--;){
			int tmp;
			fread(&tmp,sizeof(int),1,infp);
			treePos.push_back(tmp);
		}
		int firstPos, l=fileSize/interval+1;
	...
	base->nc.resize(256);
	for(int i = base->size = 256;i--;) base->nc[i] = i;
	BYTE* history = new BYTE[MAXDEPTH];
	if(stdoutCheck == false) printf("done\n");
	if(orderList.size() == 0){
